...
export function AISuggestions({ profile, latitude, longitude }: AISuggestionsProps) {
  ...
  const { data: weatherData } = useQuery({
    ...
    queryFn: async () => {
      console.log("Fetching weather (AISuggestions) from:", `${import.meta.env.VITE_URL}/api/weather?lat=${latitude}&lon=${longitude}`);
      const response = await fetch(`${import.meta.env.VITE_URL}/api/weather?lat=${latitude}&lon=${longitude}`);
      console.log("Weather API raw response (AISuggestions):", response);
      if (!response.ok) {
  ...
  const { data: suggestions, isLoading, error } = useQuery({
    queryKey: ["/api/ai/suggestions", profile, weatherData || null],
    enabled: !!(weatherData && profile.age && profile.gender && profile.occupation),
    queryFn: async () => {
      console.log("Fetching AI suggestions from:", `${import.meta.env.VITE_URL}/api/ai/suggestions`);
