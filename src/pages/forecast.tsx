...
export default function Forecast() {
  ...
  = useQuery<LocationData>({
    ...
    queryFn: () => fetch('/api/location').then(res => res.json()),
...
function ForecastContent({ period, location }: { period: string, location: any }) {
  const { data: forecast, isLoading: forecastLoading, error: forecastError } = useQuery({
    ...
    queryFn: () => fetch(`/api/forecast?lat=${location?.latitude}&lon=${location?.longitude}&period=${period}`)
      .then(res => res.json()),
    enabled: !!location,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-blue-600">
                {Math.round(forecast.data.reduce((sum: number, day: any) => sum + day.temperature, 0) / forecast.data.length)}°C
              </p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-600">Temperature Range</p>
              <p className="text-lg font-semibold text-gray-900">
                {Math.min(...forecast.data.map((d: any) => d.temperature))}° - {Math.max(...forecast.data.map((d: any) => d.temperature))}°C
              </p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-600">Prediction Confidence</p>
              <p className="text-lg font-semibold text-green-600">{forecast.confidence}%</p>
            </div>
          </div>
